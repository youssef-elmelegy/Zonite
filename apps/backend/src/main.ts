...
async function bootstrap(): Promise<void> {
  ...
  app.useLogger(app.get(Logger).setContext("NestFactory"));
  ...
  // Startup logs
  logger.log(`[startup] port: ${env.PORT}`);
  logger.log(`[startup] environment: ${env.NODE_ENV}`);
  logger.log(`[startup] log level: ${logLevel}`);
  logger.log(`[startup] docs: http://localhost:${env.PORT}/api/docs`);
}
