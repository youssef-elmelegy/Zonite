      ...
      >
        <div
          style={{
            ...
            borderRadius: 20,
            background:
              'linear-gradient(160deg, rgba(188,90,215,0.24) 0%, rgba(247,23,86,0.18) 55%, rgba(253,235,86,0.10) 100%)',
        ...
        >
          <div
            style={{
              ...
              inset: 0,
              background:
                'radial-gradient(circle at 30% 30%, rgba(253,235,86,0.15), transparent 50%)',
            }}
          />
          <div
            style={{
        ...
        return (
          <div
            ...
            style={{
              ...
              background: c,
              borderRadius: 4,
              boxShadow: st !== 'empty' ? `0 0 8px ${c}` : 'none',
            }}
          />
        );
      })}
    ...
    <div style={{ marginBottom: 16 }}>
      ...
      <div
        style={{
          ...
          boxShadow:
            focus && !error
              ? '0 0 0 3px rgba(253,235,86,0.12)'
              : error
                ? '0 0 0 3px rgba(247,23,86,0.12)'
                : 'none',
        }}
      >
        <input
      ...
      {error && (
        <div style={{ color: 'var(--fire-red)', fontSize: 11, marginTop: 6, fontWeight: 600 }}>
          {error}
        </div>
      )}
      {hint && !error && (
        <div style={{ color: 'var(--fg-tertiary)', fontSize: 11, marginTop: 6 }}>{hint}</div>
      )}
    </div>
  );
}
  ...
  const steps = [
    {
      ...
      desc: 'Tap cells faster than your rivals. The board fills one block at a time — and only the quickest take ground.',
      art: 'scatter',
      kicker: 'How it works',
    },
    {
      title: 'Play Solo or Squad',
      desc: 'Free-for-all with every player a color, or Red vs Blue team matches where every claim feeds your side.',
      art: 'split',
      kicker: 'Two ways to play',
    },
    {
      title: 'Invite with a Code',
      desc: 'Every room has a 6-character code. Share it, hit Ready, and let the clock drop.',
      art: 'key',
  ...
  return (
    ...
    >
        ...
        {steps.map((_, idx) => (
          <div
            ...
            style={{
              ...
              height: 4,
              borderRadius: 100,
              background: idx <= i ? 'var(--accent-yellow)' : 'rgba(255,255,255,0.08)',
              transition: 'all 200ms var(--ease-out)',
              boxShadow: idx === i ? '0 0 10px rgba(253,235,86,0.6)' : 'none',
            }}
          />
        ))}
      </div>
      <div style={{ display: 'flex', gap: 10 }}>
        {i > 0 && (
          <Btn variant="ghost" onClick={() => setI(i - 1)} icon={<IconArrowLeft size={14} />}>
            Back
          </Btn>
        )}
        <div style={{ flex: 1 }} />
...
function Login({ onLogin, onSwitch, onForgot }) {
  ...
  const submit = (e) => {
    ...
    setTimeout(() => {
      setBusy(false);
      onLogin({ email, name: email.split('@')[0] || 'KairosX', remember });
    }, 700);
  };
  ...
  return (
    ...
    >
      <SocialRow
        onSocial={(k) => onLogin({ email: `${k}user@zonite.gg`, name: `${k}Player`, social: k })}
      />
      <div
        style={{
          display: 'flex',
      ...
      <form onSubmit={submit}>
        ...
        <Btn
          ...
          block
          type="submit"
          icon={busy ? null : <IconArrowRight size={16} />}
          disabled={busy}
        >
      ...
      <div
        style={{ marginTop: 22, textAlign: 'center', fontSize: 13, color: 'var(--fg-tertiary)' }}
      >
        New to Zonite?{' '}
        <button
          type="button"
...
function Signup({ onSignup, onSwitch }) {
  ...
  return (
    ...
    >
      <SocialRow
        onSocial={(k) => onSignup({ email: `${k}user@zonite.gg`, name: `${k}Player`, social: k })}
      />
      <div
        style={{
          display: 'flex',
        ...
        >
          <div
            ...
            style={{
              ...
              border: `1px solid ${agree ? 'var(--accent-yellow)' : err.agree ? 'var(--fire-red)' : 'var(--border-default)'}`,
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
            }}
          ...
          <span>
            I agree to the{' '}
            <a style={{ color: 'var(--accent-yellow)', textDecoration: 'none' }}>Terms</a> and{' '}
            <a style={{ color: 'var(--accent-yellow)', textDecoration: 'none' }}>Privacy Policy</a>.
          </span>
        </label>
        {err.agree && (
          <div
  ...
  if (sent) {
    return (
      <AuthLayout
        ...
        subtitle={`We sent a 6-digit code to ${email}. Enter it on the next screen to set a new password.`}
        hero={<MiniGridArt pattern="key" />}
      >
        <div
          style={{
            padding: 16,
            background: 'rgba(75,255,84,0.08)',
        ...
        >
          ...
          <span style={{ fontSize: 13, color: 'var(--lime-300)', fontWeight: 600 }}>
            Email sent. Check spam if you don't see it.
          </span>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <Btn variant="secondary" onClick={onBack} icon={<IconArrowLeft size={14} />}>
            Back to Sign In
          </Btn>
          <div style={{ flex: 1 }} />
          <Btn variant="primary" onClick={() => onSent(email)} icon={<IconArrowRight size={14} />}>
            Enter Code
          </Btn>
        </div>
      </AuthLayout>
    ...
    >
      <form onSubmit={submit}>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 10 }}>
          <Btn variant="ghost" onClick={onBack} icon={<IconArrowLeft size={14} />}>
            Back
          </Btn>
          <div style={{ flex: 1 }} />
          <Btn variant="primary" type="submit" icon={<IconArrowRight size={14} />}>
            Send Code
          </Btn>
        </div>
      </form>
...
function Reset({ email, onDone, onBack }) {
  ...
  return (
    <AuthLayout
      ...
      subtitle={`Enter the code we sent to ${email || 'your email'} and pick a new password.`}
      hero={<MiniGridArt pattern="key" />}
    >
      <form onSubmit={submit}>
